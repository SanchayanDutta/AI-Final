...
This package contains a 25-item ANIMALS attribute table and ready-to-use prompts for running Optimal Question Asking (OQA) API experiments. It also includes an entropy plot bundle for quick visualization and benchmarking.

## Repository layout

```
25-animals-oqa-dataset/
├─ data/
│  ├─ 25_animals.json
│  ├─ items.txt
│  ├─ attributes.txt
...
├─ plots/
│  ├─ 25_animals_entropy_summary.csv
│  ├─ 25_animals_entropy_summary.json
│  ├─ 25_animals_entropy_seeds.csv
│  ├─ 25_animals_entropy_plot.png
│  └─ make_plot.py
├─ prompts/
│  ├─ prompt_plain.txt
...
## Contents
- `data/25_animals.json` — canonical boolean attribute table (25 animals × attributes).
...
- `plots/25_animals_entropy_summary.csv` — tidy table with columns: model, step, entropy_bits_mean, entropy_bits_std, entropy_bits_lo, entropy_bits_hi.
- `plots/25_animals_entropy_summary.json` — JSON version of the same table plus a small metadata block.
- `plots/25_animals_entropy_seeds.csv` — per-turn samples for variation analysis (4 models × 7 steps × 10 seeds = 280 rows).
...
3. Loop:
   ...
   - Answer "Yes" or "No" from `data/25_animals.json` for the hidden target.
   - Feed the answer back to the model.
   - Stop when only one candidate (or one equivalence class) remains.

## Plot bundle usage
- The figure `plots/25_animals_entropy_plot.png` visualizes entropy in bits across steps for four series: GPT 4.1, Gemini 2.0 Flash, Claude Haiku 4.5, and Oracle.
- The CSV in `plots/25_animals_entropy_summary.csv` drives the plot.
